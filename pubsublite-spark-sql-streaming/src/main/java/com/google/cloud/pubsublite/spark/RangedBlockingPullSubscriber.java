...
public class RangedBlockingPullSubscriber implements AutoCloseable {
    ...
    private SparkPartitionOffset endOffset;

    public RangedBlockingPullSubscriber(
    ...
    public SequencedMessage pull() throws InterruptedException, CheckedApiException {
        ...
        try {
            ...
            if (msg.offset().value() > endOffset.offset()) {
                underlying.stopAsync();
                return null;
            } else if (msg.offset().value() == endOffset.offset()) {
                underlying.stopAsync();
            } else {
                underlying.allowFlow(
                        FlowControlRequest.newBuilder()
